...
test('explicit timezone represents the same instant as UTC',()=>{assert.equal(end,Date.parse('2026-10-01T05:00:00Z'));});
...
test('timezone-less, invalid target and invalid current time are rejected',()=>{assert.throws(()=>countdownValues('2026-10-01T00:00:00',end));assert.throws(()=>countdownValues('brokenZ',end));assert.throws(()=>countdownValues(festival.announcementAt,NaN));});
